...
//
// Містить базові структури даних, які використовують ВСІ
// компоненти (інтерфейси, реалізації, MissionProcessor):
//   - Coord       — 2D-координата з перевантаженими операторами
//   - AmmoParams  — параметри боєприпасу (маса, drag, lift)
//   - Target      — ціль (позиція + швидкість у конкретний момент)
//   - DropPoint   — результат розрахунку: точка скиду + метадані
//   - MissionConfig — конфіг місії (читається IConfigLoader)
//
// Допоміжні вільні функції: length, normalize, distance, normalizeAngle.
// ============================================================
...
// Зберігся повний інтерфейс із ДЗ3, бо ним користуються всі модулі.
// ------------------------------------------------------------
struct Coord {
    float x;
    float y;
...
// Звичайний POD: name (char[32]), mass, drag, lift.
// ------------------------------------------------------------
struct AmmoParams {
    char  name[32];
    float mass;
