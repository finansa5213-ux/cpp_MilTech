...
//
// Зауваження щодо завантаження цілей:
// Інтерфейс ITargetProvider навмисно мінімальний (getTargetCount/getTarget),
// як того вимагає ТЗ. Конкретні реалізації можуть потребувати
// додаткового кроку завантаження (JsonTargetProvider::loadFromFile).
// MissionProcessor покладається на те, що ЗОВНІШНІЙ код (main)
// вже виконав цей крок ПЕРЕД передачею провайдера сюди.
// Якщо хочеться зробити це автоматично — у наступному рефакторингу
// можна додати в інтерфейс метод init() з default no-op реалізацією.
// ============================================================
