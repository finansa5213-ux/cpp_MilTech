...
int main(int argc, char** argv)
{
  ...
  try {
    ...
    std::cout << "MAVLink 2 -> " << options.dest.host << ":" << options.dest.port << " | ціль (" << options.drone.target_x_m << ", "
              << options.drone.target_y_m << ") м, висота " << options.drone.cruise_alt_m << " м\n";
    ...
    while (sim_time_s < kMaxFlightTimeS) {
      ...
      if (release_now) {
        ...
        std::cout << "[скид] t=" << sim_time_s << " с, lat=" << point.lat_deg << ", lon=" << point.lon_deg << ", alt=" << drone.altitude()
      ...
      if (!drop_reported && link.dropFinished()) {
        ...
        if (link.dropState() == mavtel::DropState::Acked) {
          ...
          std::cout << "[скид] ACK не отримано після " << link.dropAttempts() << " спроб\n";
        }
      }
      if (drop_reported) {
