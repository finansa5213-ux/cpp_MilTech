...
MavlinkLink::MavlinkLink(ITransport& transport, std::uint8_t sysid, std::uint8_t compid)
    : transport_(transport), sysid_(sysid), compid_(compid) {
    ...
    tx_status->flags = static_cast<std::uint8_t>(tx_status->flags &
                                                 ~MAVLINK_STATUS_FLAG_OUT_MAVLINK1);
}

void MavlinkLink::sendHeartbeat() {
    mavlink_message_t message{};
    mavlink_msg_heartbeat_pack(sysid_, compid_, &message,
                               MAV_TYPE_QUADROTOR,
