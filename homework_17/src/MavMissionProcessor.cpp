// ============================================================
// MavMissionProcessor.cpp — автопілот ДЗ11 + телеметрія MAVLink 2.
// Відмінності від ДЗ11 позначені міткою [ДЗ17].
// ============================================================
...
MavMissionProcessor::MavMissionProcessor(UartLink& uart, GpioSignals& gpio,
                                         LinkState& state,
...
// ------------------------------------------------------------
void MavMissionProcessor::runRx()
{
    dlink::Parser parser;
    uint8_t buf[256];
    uint8_t payload[260];
...
// ------------------------------------------------------------
bool MavMissionProcessor::pickTarget(const std::map<int, TargetTrack>& targets,
...
// Основний цикл місії.
// ------------------------------------------------------------
void MavMissionProcessor::runMission()
{
    ...
    while (!st_.stop.load()) {
        ...
        // -------- чекаємо нову телеметрію / RESULT --------
        {
            ...
            if (!woke) { // тайм-аут без нових даних
                lk.unlock();
                bridge_.tick(mavtel::nowMs()); // [ДЗ17] HEARTBEAT + повтори ACK
        ...
        // [ДЗ17] Той самий кадр, що веде місію, йде і в MAVLink по UDP.
        bridge_.onTelemetry(t, mavtel::nowMs());
        ...
        if (!dropped_ && dp.valid && t.speed > 0.5f) {
            ...
            if (miss <= margin) {
                ...
                bridge_.onDrop(t, mavtel::nowMs()); // [ДЗ17] COMMAND_LONG
                dropped_ = true;
                dropTms_ = t.t_ms;
    ...
    // [ДЗ17] Телеметрія ДЗ11 могла обірватись одразу після скиду, а на 5 спроб
    // COMMAND_LONG треба до 2.5 с. Добиваємо їх у режимі вибігу.
    coastUntilDropResolved();
}

// ------------------------------------------------------------
// [ДЗ17] Вибіг: потік MAVLink не зупиняється, доки не вирішилась доля
// COMMAND_LONG (ACK або вичерпані спроби). Позиція екстраполюється по
// останній швидкості, тому «зміна позиції vs швидкості» лишається узгодженою.
// ------------------------------------------------------------
void MavMissionProcessor::coastUntilDropResolved()
{
    if (!bridge_.hasSample()) return;

    constexpr auto kGrace   = 700ms;  // трохи телеметрії після розв'язки
    constexpr auto kMaxWait = 8s;     // запобіжник

    const auto started = steady_clock::now();
    auto resolvedAt = steady_clock::time_point{};

    while (steady_clock::now() - started < kMaxWait) {
        bridge_.coast(mavtel::nowMs());

        if (bridge_.dropFinished()) {
            if (resolvedAt == steady_clock::time_point{})
                resolvedAt = steady_clock::now();
            else if (steady_clock::now() - resolvedAt > kGrace)
                break;
        }
        std::this_thread::sleep_for(20ms);
    }

    if (bridge_.dropState() == mavtel::DropState::Acked)
        std::printf("[mavlink] ACK otrymano (sprob: %d)\n", bridge_.dropAttempts());
    else if (bridge_.dropState() == mavtel::DropState::Failed)
        std::printf("[mavlink] ACK ne otrymano pislya %d sprob\n", bridge_.dropAttempts());
}
