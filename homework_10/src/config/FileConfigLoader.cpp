...
bool FileConfigLoader::loadConfigFile()
{
    ...
    try {
        ...
        // simTimeStep — наявний параметр (сумісність: timeStep / simTimeStep)
        if      (sim.contains("simTimeStep")) cfg_.simTimeStep = sim.at("simTimeStep").get<float>();
        else if (sim.contains("timeStep"))    cfg_.simTimeStep = sim.at("timeStep").get<float>();
        else                                  cfg_.simTimeStep = 0.05f;
        cfg_.hitRadius      = sim.contains("hitRadius")
                              ? sim.at("hitRadius").get<float>() : 2.0f;

        // --- ДЗ10: нові параметри (дефолти, якщо відсутні) ---
        cfg_.physicsTimeStep = sim.contains("physicsTimeStep")
                               ? sim.at("physicsTimeStep").get<float>() : 0.01f;
        cfg_.targetTimeStep  = sim.contains("targetTimeStep")
                               ? sim.at("targetTimeStep").get<float>() : 0.05f;
        cfg_.timeScale       = sim.contains("timeScale")
                               ? sim.at("timeScale").get<float>() : 1.0f;
        if (cfg_.physicsTimeStep <= 0.f) cfg_.physicsTimeStep = 0.01f;
        if (cfg_.timeScale       <= 0.f) cfg_.timeScale       = 1.0f;
