...
DropPoint MissionProcessor::step()
{
    ...
    if (!dp.valid) {
        ...
        recoverFromError(drone_, provider_.get(), currentIdx_,
                         skippedTargets_);
        return dp;
    }
    ...
    std::cout << "  [SM] Ціль #"  << currentIdx_
              ...
              << ", "             << drone_.data().pos.y << ")"
              << "  стан="        << drone_.stateName() << "\n";

    // --- 3. Симуляційний цикл ---
    std::string prevState = drone_.stateName();
    int ticks = 0;

    while (!drone_.isTerminal())
    {
        drone_.tick(cfg_.simTimeStep);
        ++ticks;

        if (prevState != drone_.stateName()) {
            std::cout << "    [SM] " << prevState
                      << " → "      << drone_.stateName()
                      << "  t="     << drone_.data().simTime << " с"
                      << "  pos=("  << drone_.data().pos.x
                      << ", "       << drone_.data().pos.y << ")"
                      << "  spd="   << drone_.data().speed  << " м/с\n";
            prevState = drone_.stateName();
        }
    }
